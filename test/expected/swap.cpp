...
static_assert(errors::detail::is_swappable<no_throw>::value, "");

template <class T1, class T2> void swap_test() {
  std::string s1 = "abcdefghijklmnopqrstuvwxyz";
  std::string s2 = "zyxwvutsrqponmlkjihgfedcba";

  errors::expected<T1, T2> a{s1};
  errors::expected<T1, T2> b{s2};
  swap(a, b);
  REQUIRE(a->i == s2);
  REQUIRE(b->i == s1);

  a = s1;
  b = errors::unexpected<T2>(s2);
  swap(a, b);
  REQUIRE(a.error().i == s2);
  REQUIRE(b->i == s1);

  a = errors::unexpected<T2>(s1);
  b = s2;
  swap(a, b);
  REQUIRE(a->i == s2);
...
TEST_CASE("swap") {
  ...
  errors::expected<no_throw, willthrow_move> a{s1};
  errors::expected<no_throw, willthrow_move> b{errors::unexpect, s2};
  should_throw = 1;
