...
#if optional_CPP11_OR_GREATER
    void use_optional( errors::optional<Implicit> ) {}
#else
    template< typename T >
    void use_optional( T ) {}
#endif
...
CASE( "optional: Allows to in-place construct from literal value (C++11, 6)" )
{
    ...
    optional<pair_t> a( bp11_in_place, 'a', 7 );
...
CASE( "optional: Allows to in-place copy-construct from value (C++11, 6)" )
{
    ...
    optional<pair_t> a( bp11_in_place, c, s );
...
CASE( "optional: Allows to in-place move-construct from value (C++11, 6)" )
{
    ...
    optional<pair_t> a( bp11_in_place, c, std::move( s ) );
...
#if optional_CPP11_OR_GREATER
    S s( 7 );
    optional<InitList> a( bp11_in_place, { 7, 8, 9, }, 'a', s );
...
#if optional_CPP11_OR_GREATER
    S s( 7 );
    optional<InitList> a( bp11_in_place, { 7, 8, 9, }, 'a', std::move( s ) );
...
#else
    using ::errors::optional_lite::detail::alignment_of;
#endif
     ...
     "[.]" )
{
    std::cout <<
#if !optional_USES_STD_OPTIONAL
        "sizeof( errors::optional_lite::detail::storage_t<char> ): " <<
         sizeof( errors::optional_lite::detail::storage_t<char> )    << "\n" <<
#endif
...
//        nonstd_lite_in_place_type_t(int),
        static_cast< errors::in_place_t >( bp11_in_place ),
#endif
        nullopt
    );
